...
namespace BlabberApp.DataStore.Plugins
{
    public class MySqlUser : IUserPlugin
    {
        ...
        public MySqlUser()
        {
            _dcUser = new MySqlConnection("server=142.93.114.73;database=JPD333;user=JPD333;password=letmein");
            try
            {
                _dcUser.Open();
            }
        ...
        public void Update(IEntity obj)
        {
            ...
            try
            {
                string sql = $"UPDATE users SET users.email='{user.Email}' WHERE users.sys_id='{user.Id}'";
                MySqlCommand cmd = new MySqlCommand(sql, _dcUser);
                cmd.ExecuteNonQuery();
            }
            catch (Exception ex)
